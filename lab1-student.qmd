...
*`ToothGrowth` is a data frame with 60 observations and 2 variables.*
...
```
```{r}
list("We fail to reject the null hypothesis due to the high p-value. The high p-value suggests that there is not enough support to show that the supplement type has a significant effect on teeth length")
list("The interpretation of the confidence interval in this case is that mean differences between the supplement types on teeth length are between -.17 and 7.57 mm. Because this interval includes 0, we do not have the evidence to support that there is a statistically significant difference in the means ")
```